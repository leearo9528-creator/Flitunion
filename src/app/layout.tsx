import type { Metadata } from "next";
import { Geist } from "next/font/google";
import GoogleAnalytics from "@/components/GoogleAnalytics";
import "./globals.css";

const geist = Geist({
  variable: "--font-geist-sans",
  subsets: ["latin"],
});

export const metadata: Metadata = {
  title: "플릿 유니온(Flit Union) | 행사 대행 · 기획부터 현장 운영·정산까지",
  description:
    "검증된 셀러 네트워크 기반 행사 대행 전문 기업. 대학 축제, 지자체 축제, 기업 행사, 플리마켓·야시장을 기획부터 셀러·푸드트럭 섭외, 장비 렌탈, 현장 운영, 정산까지 원스톱으로 대행합니다.",
  keywords: [
    "행사 대행",
    "행사 기획 대행",
    "행사 운영 대행",
    "축제 부스 운영 대행",
    "지자체 축제 대행",
    "대학교 축제 대행",
    "기업 행사 대행",
    "플리마켓 운영 대행",
    "야시장 기획",
    "푸드트럭 섭외",
    "행사 장비 렌탈",
    "셀러 모집 대행",
    "유휴공간 활용",
    "플릿 유니온",
    "Flit Union",
    "플릿",
  ],
  authors: [{ name: "플릿 유니온(Flit Union)" }],
  creator: "플릿 유니온(Flit Union)",
  publisher: "플릿 유니온(Flit Union)",
  openGraph: {
    type: "website",
    locale: "ko_KR",
    url: "https://flitunion.com",
    siteName: "플릿 유니온(Flit Union)",
    title: "플릿 유니온(Flit Union) | 행사 대행 · 기획부터 현장 운영·정산까지",
    description:
      "검증된 셀러 네트워크 기반 행사 대행 전문 기업. 대학 축제·지자체 축제·기업 행사 기획부터 셀러 섭외, 장비, 현장 운영, 정산까지 원스톱.",
  },
  twitter: {
    card: "summary_large_image",
    title: "플릿 유니온(Flit Union) | 행사 대행 · 기획부터 현장 운영·정산까지",
    description:
      "검증된 셀러 네트워크 기반 행사 대행 전문 기업. 대학 축제·지자체 축제·기업 행사 기획부터 셀러 섭외, 장비, 현장 운영, 정산까지 원스톱.",
  },
  robots: {
    index: true,
    follow: true,
    googleBot: {
      index: true,
      follow: true,
      "max-video-preview": -1,
      "max-image-preview": "large",
      "max-snippet": -1,
    },
  },
  alternates: {
    canonical: "https://flitunion.com",
  },
};

export default function RootLayout({
  children,
}: Readonly<{
  children: React.ReactNode;
}>) {
  return (
    <html lang="ko" className={`${geist.variable} h-full antialiased`}>
      <body className="min-h-full flex flex-col">
        <GoogleAnalytics />
        {children}
      </body>
    </html>
  );
}
