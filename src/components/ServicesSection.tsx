      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {services.map((service, index) => (
            <article
              key={index}
              className="group bg-gray-50 hover:bg-brand-light rounded-2xl p-6 border border-gray-100 hover:border-brand-border transition-all hover:-translate-y-1"
            >
              {/* Icon */}
              <div className="inline-flex items-center justify-center w-12 h-12 rounded-xl mb-5 bg-brand-light text-brand group-hover:bg-brand group-hover:text-white transition-colors">
                {service.icon}
              </div>
              ...
              <ul className="flex flex-wrap gap-2 mb-5" aria-label="서비스 특징">
                {service.tags.map((tag) => (
                  <li key={tag} className="text-xs text-gray-500 bg-white border border-gray-200 px-3 py-1 rounded-full">
                    {tag}
                  </li>
                ))}
              </ul>
              ...
              {/* Highlight */}
              <div className="flex items-center gap-2 text-sm font-semibold text-brand bg-white border border-brand-border rounded-lg px-4 py-2.5 mb-4">
        ...
        <div className="mt-10 bg-gray-900 rounded-2xl p-8">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
            ...
            <a
              href="#contact"
              className="shrink-0 inline-flex items-center gap-2 px-6 py-3 bg-brand hover:bg-brand-dark text-white font-semibold rounded-xl transition-colors text-sm"
            >
              통합 견적 문의
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" />
