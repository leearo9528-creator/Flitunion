/**
 * 라인 아이콘 공용 컴포넌트 (heroicons outline 스타일).
 *
 * ⚠️ 카드·배지 아이콘에 이모지를 쓰지 않는다 — CLAUDE.md 의 카피 톤 규칙 참조.
 * 새 아이콘이 필요하면 아래 ICONS 에 heroicons outline 의 path `d` 값을 추가한다.
 */
...
export const ICONS = {
  ...
  storefront: "M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4",
...
/** 연한 파란 배경의 정사각 아이콘 배지 — 카드 상단에 쓰는 기본형 */
export function IconBadge({ path, size = "md" }: { path: string; size?: "sm" | "md" }) {
  const box = size === "sm" ? "w-9 h-9" : "w-11 h-11";
  const icon = size === "sm" ? "w-5 h-5" : "w-6 h-6";
  return (
    <span
      className={`inline-flex items-center justify-center ${box} rounded-xl shrink-0`}
